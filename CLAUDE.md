...
src/
  ...
  ui/
    ...
    onboarding/ # wizard: Connect → Models (ELO step hidden 2026-08-11 — OnboardingElo stays in the tree unrendered; elo state still feeds prompts at its stored/default value); Models step has a «Recommended models» button (RecommendedModelsDialog) + yellow star badges for tested models, driven by recommendedModels.json (curated rating, RU/EN comments in the file — see the 2026-08-13 recommended-models spec)
...
Move selection now has four real per-model adapters (the first three tuned with Prompt Lab, the fourth fixed by its finetune): `gemma4` (legal-move-list prompt + `reasoningEffort: 'none'`; campaign: match 9.8% vs 7.8% baseline, legality 94.7% vs 60.5%), `qwen35` (two-stage: PGN-completion first attempt, ELO-persona legal-list chat on retries; campaign: 10.7% vs 3.3% match), `qwen25` (legal-list chat, **no** `reasoningEffort` — qwen2.5-instruct is not a reasoning model; campaign: 5.2% vs 1.5% match, legality 95.5% vs 21.2%; ships the race's runner-up v1 over winner v7 deliberately — the match gap is under 1σ while v7 costs 2.8× latency and is less legal), and `chessLm` (Alpaca completion prompt for `ippity/chessLM-0.01-llama-3.1-8b`; no campaign—format fixed by the finetune per its HF model card) — see docs/prompt-lab/*.md. The two qwen adapters match on strictly disjoint id substrings (`qwen2.5` / `qwen3.5`) because their campaigns landed on opposite prompt formats — a mis-route would silently halve play strength rather than error.
...
Small deferred follow-ups (non-blocking, from final reviews): a11y — neither modal dialog (`PromotionPicker`, `RecommendedModelsDialog`) moves or traps keyboard focus, and the popup's `.rec-list` `<ul>` needs `role="list"`; DRY — `genericFen.ts` still carries its own copy of the SAN regex instead of sharing `src/llm/adapters/parseSan.ts`.
