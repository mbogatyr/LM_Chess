...
Move selection now has three real per-model adapters (the first two tuned with Prompt Lab, the third fixed by its finetune): `gemma4` (legal-move-list prompt + `reasoningEffort: 'none'`; campaign: match 9.8% vs 7.8% baseline, legality 94.7% vs 60.5%), `qwen35` (two-stage: PGN-completion first attempt, ELO-persona legal-list chat on retries; campaign: 10.7% vs 3.3% match), and `chessLm` (Alpaca completion prompt for `ippity/chessLM-0.01-llama-3.1-8b`; no campaign—format fixed by the finetune per its HF model card) — see docs/prompt-lab/*.md.
